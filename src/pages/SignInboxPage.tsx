...
import { listMyPendingToSign, signDocument, submissionAmount } from '../data/submissions'
...
export default function SignInboxPage() {
  ...
  function reload() { if (profile) listMyPendingToSign(profile.uid).then(r => { setRows(r); setLoading(false) }) }
  ...
  // The block(s) this user must sign on a document.
  const myBlocks = (s: Submission) => (s.signatures ?? []).filter(x => x.assignedUid === profile?.uid && x.status === 'pending')
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-3">
        ...
        {!loading && <span className="rounded-full bg-amber-100 px-2.5 py-1 text-xs font-semibold text-amber-700">{rows.length}</span>}
      </div>
          ...
          <tbody className="divide-y divide-gray-100">
            {rows.map(r => (
              <tr key={r.id} className="hover:bg-gray-50 align-top">
                ...
                <td className="px-3 py-2">{myBlocks(r).map(b => b.blockLabel).join(', ')}</td>
